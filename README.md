...
```bash
$ mkdir project-name && cd project-name && composer create-project --repository-url="/root/path/not/relative/path/to/project-dir/packages.json" vigetlabs/wordpress-site-starter . --stability=dev --remove-vcs --no-install
```
