...
| `<function-prefix>` | Set from the project slug at bootstrap, with a trailing `_` (e.g. `wp-starter` → `wpstarter_`). Some projects also use a no-dash form for Twig fn names — see "Naming conventions" below. |
...
Twig function naming: `<function-prefix><blockslugnodashes>_<functionname>` (e.g. for text domain `wp-starter` and block `alert-banner`: `wpstarter_alertbanner_my_helper`). Match whatever the rest of the theme is using — read existing `block.php` files first.
...
| Twig function names | `<function-prefix><slugnodashes>_<fn>` | `wpstarter_mynewblock_get_id` |
